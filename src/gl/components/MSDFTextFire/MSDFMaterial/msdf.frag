...
pStrokeOUT = smoothstep(pStrokeOUT, pStrokeOUT + 0.5, vLayoutUv.x);
float mixStrokeOUT =  clamp(2.0 * (pStrokeOUT), 0.0, 1.0); 
//float mixStroke = abs(mixStrokeIN - (1.0 - mixStrokeOUT));
vec3 strokeColor = mix(vec3(1.0, 1.0, 1.0),vec3(0.8,0.8,0.8),mixStrokeOUT); 
