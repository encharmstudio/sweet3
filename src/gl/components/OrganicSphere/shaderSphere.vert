...
float distorted_position(vec3 p){
  float n = cnoise(p*1.0 + vec3(time*0.001));
  //float n = cnoise(p * 5.0 + vec3(1000.));
  return n;
}
...
void main() {
  ...
  vNormal = normal;
  vPosition = position;
  vec3 dipslacedposition = position + amp * normal * distorted_position(position);
  ...
  vec3 displacedPositionA = positionA + amp * normal * distorted_position(positionA);

  vec3 positionB = position + biTangent.xyz * distanceB;
  vec3 displacedPositionB = positionB + amp * normal * distorted_position(positionB);
