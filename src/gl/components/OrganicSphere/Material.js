  ...
  return 2.2 * n_xyzw;
    ...
    float amp = 0.5;
     ` + shader.vertexShader;
      ...
      ///////////////////////////////////////////////////////////////////////////////////
      // vec4 tangent = vec4(ortohonal(transformedNormal),1.0);

      // // Bi tangents
      // float distanceA = (M_PI * 2.0) / uSubdivision.x;
      // float distanceB = M_PI / uSubdivision.x;

      // vec3 biTangent = cross(transformedNormal, tangent.xyz);

      // vec3 positionA = position + tangent.xyz * distanceA;
      // vec3 displacedPositionA = positionA + amp * transformedNormal * distorted_position(positionA);

      // vec3 positionB = position + biTangent.xyz * distanceB;
      // vec3 displacedPositionB = positionB + amp * transformedNormal * distorted_position(positionB);

      // vec3 computedNormal = cross(displacedPositionA - dipslacedposition.xyz, displacedPositionB - dipslacedposition.xyz);
